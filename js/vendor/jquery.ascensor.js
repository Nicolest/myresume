...
(function($, window, undefined) {
  ...
  var pluginName = 'ascensor',
    defaults = {
      ...
      direction: "y",               // specify if direction is x,y or chocolate, this is the default direction
	  directionOnSize: null,	    // Define another direction under a width/height size threshold : {type:width or height, size:value, direction:x, y or chocolate} (ex {type: 'width', size:300, direction:"x"})
      ...
      overflow: "scroll",
	  position: {left:0, right:0, bottom:0, top:0}, // Absolute position of the ascensor container, values are top/bottom/left/right (ex : {left:x, top:y})  
	  positionOnSize: {type:'width', size:0, position:{left:0, right:0, bottom:0, top:0}}, // Define another absolute position under a width/height size threshold (ex {type:'width', size:300, position:{left:x, top:y}})
	  minWidth:null,
	  minHeight:null
    };
  ...
  Plugin.prototype.init = function() {
	...
	definePosition(self.options.position);
	...
	function definePosition(position){
		$(node).css("top", position.top);
		$(node).css("bottom", position.bottom);
		$(node).css("left", position.left);
		$(node).css("right", position.right);
	}
    ...
    function resize() {
	  ...
	  // redefine the direction if necessary
	  if(self.options.directionOnSize != null){
	    var resize = false;
		if(self.options.directionOnSize.type == 'width'){
			if(windowWidth <= self.options.directionOnSize.size){
				resize = true;
			}
		}
		if(self.options.directionOnSize.type == 'height'){
			if(windowHeight <= self.options.directionOnSize.size){
				resize = true;
			}
		}
		if(resize){
			self.options.direction = self.options.directionOnSize.direction;
		}
		else{
	  ...
	  // redefine the position if necessary
	  if((self.options.positionOnSize.type == 'width' && windowWidth <= self.options.positionOnSize.size)
	    ||(self.options.positionOnSize.type == 'height' && windowHeight <= self.options.positionOnSize.size)){
		position = self.options.positionOnSize.position;
	  }
	  definePosition(position);
  
      // set a min width if necessary
	  var width = windowWidth;
	  if(self.options.minWidth != null){
		if(windowWidth < self.options.minWidth){
	  ...
	  if(self.options.minHeight != null){
		if($(window).height() < self.options.minHeight){
			height = self.options.minHeight;
		}
	  }
	  
      //update WW & WH variables
      WW = width - position.left - position.right;
      WH = height - position.top - position.bottom;
